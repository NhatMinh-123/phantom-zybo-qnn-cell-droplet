...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare two overlay frame folders.")
...
def crop_image(path: Path, crop: tuple[int, int, int, int]) -> np.ndarray:
...
def title_image(image: np.ndarray, title: str) -> np.ndarray:
    result = image.copy()
    cv2.rectangle(result, (0, 0), (result.shape[1], 28), (12, 12, 12), -1)
    cv2.putText(
        result,
        title,
        (8, 20),
...
def main() -> None:
    ...
    for frame_index in args.frames:
        ...
        left = title_image(crop_image(args.left_dir / name, crop), args.left_title)
        right = title_image(crop_image(args.right_dir / name, crop), args.right_title)
