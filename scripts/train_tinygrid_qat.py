#!/usr/bin/env python3
"""Quantization-aware training for the FPGA TinyGridNet model."""
...
from tinygrid_qnn.model import TinyGridNetFP32, TinyGridNetQNN, count_parameters
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--first-layer-weight-bits", type=int, default=8)
    parser.add_argument("--last-layer-weight-bits", type=int, default=8)
...
def transfer_fp32_weights(fp32: TinyGridNetFP32, qnn_model: TinyGridNetQNN) -> None:
    with torch.no_grad():
        for block_index in range(1, 5):
            source = getattr(fp32, f"block{block_index}")
            target = getattr(qnn_model, f"block{block_index}")
            target.conv.weight.copy_(source[0].weight)
            target.bn.load_state_dict(source[1].state_dict())
        qnn_model.head.weight.copy_(fp32.head.weight)
        qnn_model.head.bias.copy_(fp32.head.bias)
...
def loss_value(
    ...
    return bce + dice_weight * (1.0 - (numerator / denominator).mean())
...
def report_at_half(
    targets: np.ndarray, probabilities: np.ndarray, config: TinyGridConfig
) -> dict[str, object]:
...
def plot_history(history: list[dict[str, float]], output: Path) -> None:
    ...
    axes[1].set_title("QAT validation macro F1 at 0.50")
...
def main() -> None:
    ...
    fp32_checkpoint = torch.load(
        args.fp32_checkpoint.resolve(), map_location=device, weights_only=False
    )
    feature_mode = str(fp32_checkpoint["feature_mode"])
    input_channels = len(FEATURE_MODES[feature_mode])
    fp32_model = TinyGridNetFP32(config, input_channels=input_channels).to(device)
    fp32_model.load_state_dict(fp32_checkpoint["model_state"])
    ...
    positive_weight = np.clip((total - positive) / np.maximum(positive, 1), 1.0, 20.0)
    ...
    model = TinyGridNetQNN(
        ...
        first_layer_weight_bits=args.first_layer_weight_bits,
        last_layer_weight_bits=args.last_layer_weight_bits,
        output_bits=args.output_bits,
    ).to(device)
    transfer_fp32_weights(fp32_model, model)
    quantization = dict(model.quantization)
    total_parameters, trainable_parameters = count_parameters(model)

    with torch.no_grad():
        initial_loss, initial_targets, initial_probabilities = run_epoch(
            model, loaders["valid"], criterion, device, args.dice_weight
        )
    initial_report = report_at_half(initial_targets, initial_probabilities, config)
    print(
        f"device={device} feature_mode={feature_mode} quantization={quantization} "
        f"initial_valid_f1={float(initial_report['macro_f1']):.4f}",
        flush=True,
    )
    ...
    best_f1 = -1.0
    stale_epochs = 0
    best_path = output / "best_qat.pt"
    started = time.perf_counter()
    for epoch in range(1, args.epochs + 1):
        train_loss, train_targets, train_probabilities = run_epoch(
            model, loaders["train"], criterion, device, args.dice_weight, optimizer
        )
        with torch.no_grad():
            ...
            torch.save(
                {
                    ...
                    "source_fp32_checkpoint": str(args.fp32_checkpoint.resolve()),
                    "epoch": epoch,
    ...
    with (output / "training_metrics.csv").open("w", newline="", encoding="ascii") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(history[0]))
        writer.writeheader()
        writer.writerows(history)
    plot_history(history, output / "training_curves.png")
    result = {
        "model": "TinyGridNetQNN two-channel occupancy",
        "feature_mode": feature_mode,
        "channel_indices": list(FEATURE_MODES[feature_mode]),
        "quantization": quantization,
        ...
        "initial_validation_at_half": initial_report,
    ...
    (output / "metrics_qat.json").write_text(json.dumps(result, indent=2), encoding="ascii")
